...
叶无痕凝视那行`template<typename T>`，久久不语。
...
他想起C++标准库中那个他从未真正用过的`std::vector<T>`——它也有默认模板参数，只是藏得太深，修士们大多不知。
...
她望向那行`template<> class Sword<JianXiu>`。
...
他找到了叶沧澜藏在剑中的遗言，得知《异派标准》的终章藏于`std::vector<bool>`之中。
...
他要找到叶沧澜藏在`std::vector<bool>`里的异派标准终章。

然而`vector<bool>`——这个C++标准库中最诡异的容器，不存储bool，存储位。它的迭代器不返回bool&，返回一个神秘的代理对象。
...
第六卷（大乘飞升·异派标准）将聚焦`vector<bool>`特化、代理对象、分配器、移动语义、完美转发等现代C++核心特性，并收束叶沧澜千年前的遗愿，预计约2万字。
